...
private double[][]
    ...
    /**
     * the travelTo function takes a waypoint as input. It retrieve the current robot's coordinates using the odometer and calculates the difference of the waypoint's coordinates
     * and the robot coordinates to get the distance it still has to travel to reach the next waypoint. 
     * Depending on the deltaX and deltaY coordinate we can know in which quadrant the next waypoint is located. 
     * 
     * then using the Math.atan the angle theta ( the direction of where the next waypoint is)  will be calculated, it will then be substracted from the current robot's heading and we will therefore
     * know the correction we have to apply to our robot's heading in order to reach the next waypoint 
     * Finally the turnTo() method is called to get the minimal angle correction. 
     * 
     * @param x
     * @param y
     */
