...
//    EV3ColorSensor colSensor = new EV3ColorSensor(lsPort);
     ...
     * This method makes the robot travel to the origin, apply a correction algorithm, and stop at origin in correct angle finally.
     * 
     *  it starts by orienting the robot towards the origin, it then makes the robot travels to the origin. Once the robot reaches the origin(by detecting a black line), it makes  it stop then 
     *  move backwards by the SENSOR_OFFSET value. 
     *  Then it will make the robot rotate clockwise until it detects 4 black lines, and store the robot's theta value each time it 
     *  crosses one. Then by a small series of calculations(detailed below) the robot's position relative to the origin is found and we can now make the robot travel to the origin. 
     *  Finally all that is left to do is to correct the heading of the robot, this is done by reading the angle reported by the odometer and make the robot rotate by the negative of this value, this way 
     *  the robot will be on the 0-degree axis. 
     *  
     *  
     */
    ...
    /**
     * 
     * this methods reads the robot's current position from the odometer , and calculates how far it needs to move based on that
     * 
     ...
     * ex: if thetaDiff indicates that the robot should turn 240 degree in a certain  direction. It is the same as turning 120 degree in the other direction. We therefore, 
     * minimize the angle
     * 
    ...
    /**
     * This is a simple getter function that gets the value read by the 
     * light sensor and simply returns it .
    ...
    /**
     * this is a filter that we use for our light sensor, it takes in 5 successive values read by the light sensor and filters them
     ...
     *  they convert distance or angle respectively into the appropriate number of tacho counts the motor has to perform. 
     *  
     *  
     * @param radius
     * @param distance
